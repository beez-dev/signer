...
export function FileInput({ownerEmail}: FileInputProps) {
    ...
    const [allRecords, setAllRecords] = useState<{ fileName: string, status: Record<string, string> }[]>([]);
            ...
            {allRecords.map((eachRecord, idx) => {
                ...
                return <div className="border border-gray-300 min-w-32 p-8 rounded-2xl" key={idx}>
                    ...
                    <div className="text-gray-400">
                        {Object.values(eachRecord.status).filter((e) => e === 'accepted').length} out
                        of {Object.values(eachRecord.status).length} people have accepted invitation
                    </div>
                </div>
            })}
        </div>
